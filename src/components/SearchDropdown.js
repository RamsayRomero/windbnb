...
const SearchDropdown = ({
  ...
  return (
    <CSSTransition
      ...
      classNames='dropdown'
    >
      <div className='z-20 py-4 fixed inset-0 block w-full bg-white font-display md:inset-x-0 md:top-0 md:bottom-auto md:h-128'>
        <div className='flex px-4 items-center justify-between md:hidden'>
          <div className='font-bold text-sm'>Edit your search</div>

          <button
            className='block rounded focus:outline-none focus:ring-1 ring-gray-700'
            ...
            <ul>
              {locations.map((city) => (
                <li
                  ...
                  className='flex items-center px-10 py-2.5 mb-1 cursor-pointer hover:bg-gray-100'
                >
                  ...
                  <div>{city === 'Finland' ? city : `${city}, Finland`}</div>
                </li>
              ))}
            </ul>
          </div>
        ...
        <div className='flex justify-center w-full absolute bottom-0 mb-4 md:hidden'>
          <button
            tabIndex={!isOpen ? -1 : undefined}
            onClick={() => {
              setIsOpen(false);
